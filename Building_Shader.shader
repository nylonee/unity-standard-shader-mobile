Shader "Custom/Building (Supports Lightmap)" {
	Properties{
		...
		_DetailStrength("Strength of detail", Range(-3.0, 3.0)) = 0
	}
	...
	{
		half4 main_color = brightness_contrast(tex2D(_MainTex, i.uv_main) + _DetailStrength * tex2D(_DetailTex, i.uv_main));
		// Apply color tint
